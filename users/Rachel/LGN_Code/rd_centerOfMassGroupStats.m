...
tal3Ts.vt0.centers1Thresh = [c10(:,:,1); c10(:,:,2).*repmat([-1 1 1],nSubjects3T,1)];
tal3Ts.vt0.centers2Thresh = [c20(:,:,1); c20(:,:,2).*repmat([-1 1 1],nSubjects3T,1)];
tal3Ts.vtT.centers1Thresh = [c1t(:,:,1); c1t(:,:,2).*repmat([-1 1 1],nSubjects3T,1)];
tal3Ts.vtT.centers2Thresh = [c2t(:,:,1); c2t(:,:,2).*repmat([-1 1 1],nSubjects3T,1)];
...
% for the right hemisphere, flip the x coord to create lateral-medial
% dimension instead of left-right
tal7Ts.vt0.centers1Thresh = [c10(:,:,1); c10(:,:,2).*repmat([-1 1 1],nSubjects7T,1)];
tal7Ts.vt0.centers2Thresh = [c20(:,:,1); c20(:,:,2).*repmat([-1 1 1],nSubjects7T,1)];
tal7Ts.vtT.centers1Thresh = [c1t(:,:,1); c1t(:,:,2).*repmat([-1 1 1],nSubjects7T,1)];
tal7Ts.vtT.centers2Thresh = [c2t(:,:,1); c2t(:,:,2).*repmat([-1 1 1],nSubjects7T,1)];
