function th=rotateticklabel(h,rot,opt)
...
if strcmp(opt,'image')
    if rot<180
        th=text(b,repmat(c(end)+.6*(c(2)-c(1)),length(b),1),a,'HorizontalAlignment','right','rotation',rot);
    else
        th=text(b,repmat(c(end)+.6*(c(2)-c(1)),length(b),1),a,'HorizontalAlignment','left','rotation',rot);
    end
else
    if rot<180
